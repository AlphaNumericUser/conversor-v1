...
class PresionUnitInput extends ConsumerWidget {
  const PresionUnitInput({
    Key? key,
  ...
  final List<Presion> lista;
  final double fontSize;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final presionController = ref.watch(presionControllerProvider);

    void handleInputChange(String value) {
      if (value.isNotEmpty) {
        try {
          double valorNumerico = double.parse(value);
          ref.read(presionValorNumericoProvider.notifier).cambiarValor(valorNumerico);
              ...
              child: Center(
                child: Padding(
                  ...
                  child: DropdownButtonFormField(
                    ...
                    onChanged: (value) {
                      if (value != null) {
                        ref.read(presionEntradaProvider.notifier).actualizarValor(value);
                      }
                    },
                  ),
                ),
