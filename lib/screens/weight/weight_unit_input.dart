...
class WeightUnitInput extends ConsumerWidget {
  
  const WeightUnitInput({
    Key? key,
  ...
  final List<Weight> lista;
  final double fontSize;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final pesoController = ref.watch(pesoControllerProvider);

    void handleInputChange(String value) {
      if (value.isNotEmpty) {
        try {
          double valorNumerico = double.parse(value);
          ref.read(pesoValorNumericoProvider.notifier).cambiarValor(valorNumerico);
              ...
              child: Center(
                child: Padding(
                  ...
                  child: DropdownButtonFormField(
                    ...
                    onChanged: (value) {
                      if (value != null) {
                        ref.read(pesoEntradaProvider.notifier).actualizarValor(value);
                      }
                    },
                  ),
                ),
